...
string Bunch::traverse(string directory, bool all) {
    ...
    if((dir = opendir(directory.c_str())) != NULL) {
        //Bunch newEntry = bunch.addEntry(bunch.path_, magicDir, format, all);
        ...
        while((entry = readdir(dir)) != NULL) {
            ...
            if (entry -> d_name[1] != '.' && all && !(entry->d_name[0] == '.' && entry->d_name[1] == '\0')) {
                ...
                this->entryStrings.push_back(nextFilename.str());
                this->entries.push_back(Bunch(entryStrings.back(), magicFile_, format_, all_));
